...
void SelectCarScene::onKeyReleased(EventKeyboard::KeyCode keycode, Event* event)
{
    if(EventKeyboard::KeyCode::KEY_RETURN == keycode || EventKeyboard::KeyCode::KEY_ESCAPE == keycode || EventKeyboard::KeyCode::KEY_BACKSPACE == keycode)
    {
        ...
        {
            Director::getInstance()->replaceScene(TransitionSlideInL::create(0.5f, SelectMapScene::createScene()));
        }
    }
}
...
void SelectCarScene::initUI()
{
    ...
    //daily task
    auto buttonDailyTask = Helper::seekWidgetByName(m_view, "Button_DailyTask");
    buttonDailyTask->addTouchEventListener(this, toucheventselector(SelectCarScene::dailyTaskCallback));

    auto achievementTask = Helper::seekWidgetByName(m_view, "Button_Achievement");
    achievementTask->addTouchEventListener(this, toucheventselector(SelectCarScene::achievementCallback));
...
void SelectCarScene::selectEvent()
{
    ...
    {
        Director::getInstance()->replaceScene(TransitionSlideInR::create(0.5f, UpgradeCarScene::createScene()));
    }
    else
    {
